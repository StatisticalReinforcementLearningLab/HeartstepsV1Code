...
rownames(tmp)=c("responded","connected but not responded", "disconnected", "Sum")
print(xtable(tmp), include.colnames = FALSE, hline.after = c(-1,3,4))
@

In the dataset, there are $\Sexpr{nrow(temp)}$ total person-day datapoints obtained from 37 subjects for durations ranging from $\Sexpr{min(aggregate(study.day.nogap~user, data=daily, max)[,2])+1}$ to $\Sexpr{max(aggregate(study.day.nogap~user, data=daily, max)[,2])+1}$ days. Among those $\Sexpr{nrow(temp)}$ datapoints, only $\Sexpr{sum(temp$respond)}$ contained one or more responses to at least one of either planning message or EMA questions.\\
...
with(temp1,boxplot(jbsteps~hectic,main="hectic"))
@
...
text(x = xx, y = d, label = d, pos = 3, cex = 1)
with(temp1,boxplot(jbsteps~stressful,main="stressful"))
@
...
xx=barplot(d,ylim=c(0,max(d)+200), names.arg=names(d))
text(x = xx, y = d, label = d, pos = 3, cex = 1)
with(temp1,boxplot(jbsteps~typical,main="typical"))
@
...
text(x = xx, y = d, label = d, pos = 3, cex = 1)
with(temp1,boxplot(jbsteps~energetic,main="energetic"))
...
d=table(temp$urge, useNA =  "ifany")
names(d)[is.na(names(d))]<-"NA"
xx=barplot(d, ylim=c(0,max(d)+250))
text(x = xx, y = d, label = d, pos = 3, cex = 1)
with(temp1,boxplot(jbsteps~urge,main="urge"))
@

There is no one who checked degree 5.

\section{Missing context}
Among datapoints which contain respond==TRUE, the following number of contexts are missing. 
<<missing context, echo=FALSE, results="asis" >>=
tmp=apply(subset(temp, select = c(recognized.activity:snow)), 2, function(x) sum((is.na(x) & temp$respond),na.rm=T) )
tmp=as.table(tmp[3:14])
print(xtable(t(as.table(tmp[1:5])), auto=TRUE),include.rownames = F)
print(xtable(t(as.table(tmp[6:8])), auto=TRUE),include.rownames = F)
print(xtable(t(as.table(tmp[9:12])), auto=TRUE),include.rownames = F)
@
