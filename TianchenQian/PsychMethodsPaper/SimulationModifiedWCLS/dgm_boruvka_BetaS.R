...
# I modified it so that S follows Beta distribution:
# If prevous A = 1, S ~ 2 * Beta(2, 5) - 1
# If prevous A = 0, S ~ 2 * Beta(2, 1) - 1
dgm_boruvka_BetaS <- function(sample_size, total_T) {
    theta_1 <- 0.8
    theta_2 <- 0
    beta_0 <- -0.2
    beta_1 <- 0.5
    ...
    df_names <- c("userid", "day", "Y", "A", "S", "expect_Y", "prob_A", "shape1_Beta", "shape2_Beta", "expect_Beta", "Beta", "expect_S", "epsilon")
    ...
    dta[, t, "shape1_Beta"] <- 2
    dta[, t, "shape2_Beta"] <- 1
    dta[, t, "expect_Beta"] <- dta[, t, "shape1_Beta"] / (dta[, t, "shape1_Beta"] + dta[, t, "shape2_Beta"])
    dta[, t, "expect_S"] <- 2 * dta[, t, "expect_Beta"] - 1
    dta[, t, "Beta"] <- rbeta(sample_size, shape1 = dta[, t, "shape1_Beta"], shape2 = dta[, t, "shape2_Beta"])
    dta[, t, "S"] <- 2 * dta[, t, "Beta"] - 1
    dta[, t, "prob_A"] <- pmin(0.8, pmax(0.2, expit(eta_1 * 0 + eta_2 * dta[, t, "S"])))
    ...
    for (t in 2:total_T) {
        dta[, t, "shape1_Beta"] <- 2
        dta[, t, "shape2_Beta"] <- 4 * dta[, t - 1, "A"] + 1
        dta[, t, "expect_Beta"] <- dta[, t, "shape1_Beta"] / (dta[, t, "shape1_Beta"] + dta[, t, "shape2_Beta"])
        dta[, t, "expect_S"] <- 2 * dta[, t, "expect_Beta"] - 1
        dta[, t, "Beta"] <- rbeta(sample_size, shape1 = dta[, t, "shape1_Beta"], shape2 = dta[, t, "shape2_Beta"])
        dta[, t, "S"] <- 2 * dta[, t, "Beta"] - 1
        dta[, t, "prob_A"] <- pmin(0.8, pmax(0.2, expit(eta_1 * dta[, t - 1, "A"] + eta_2 * dta[, t, "S"])))
        dta[, t, "A"] <- rbinom(sample_size, 1, dta[, t, "prob_A"])
