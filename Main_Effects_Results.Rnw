...
\footnotesize{\texttt{
\textbf{Compilation Information} \\
User: \Sexpr{as.character(Sys.info()["login"])} \\
System: \Sexpr{as.character(Sys.info()["sysname"])} version \Sexpr{as.character(Sys.info()["release"])} \\
\Sexpr{R.version$version.string}
}}

\section{Changes from Previous Versions}
\label{sec:changes}
\subsection{January 13, 2017 - July 5, 2017}
Corrected sensitivity analyses for increasing minimum-required time-on-study (see \Cref{sec:sens-analysis-min-time-on-study})
to include participant 35. 
He was previously excluded for being ``anomalous''. Pedja and Susan were unable to sufficiently justify this, so the analyses
have been adjusted so that he is included in all sensitivity analyses in \Cref{sec:sens-analysis-min-time-on-study}.
\subsection{November 21, 2016 - January 13, 2017}
Added sensitivity analyses for model 5, which is marginal over time but splits the treatment indicator into indicators for both active and anti-sedentary suggestions.
\subsection{August 8, 2016 - November 21, 2016}
A small typo was found in the \texttt{estimate} function used to extract model results. This did not affect the parameter estimates, but did lead to very small (on the order of $10^{-4}$) changes in upper and lower confidence limits for those estimates. There were small changes in $p$-values, but significance did not change for any test. The corrected results are contained herein.  
\subsection{July 15, 2016 - August 8, 2016}
Participant travel dates were changed to more accurately reflect GPS locations. Recall that travel time is entirely removed from the data used for analysis.
\begin{enumerate}
  \item Previously, participant 14 was marked as traveling from 9/18/2015 - 9/24/2015. These dates were chosen basded on knowledge that the participant traveled to Hawaii during the study and the fact that she was missing Jawbone step counts over this period. However, GPS data indicates that the participant was in Ann Arbor duing this time, but in Hawaii from 9/1/2015 - 9/5/2015. The participant's data was changed to exclude 9/1/2015 - 9/5/2015, and include 9/18/2015 - 9/24/2015.
  \item Participant 18 was previously never marked as traveling, despite reporting a trip to Michigan's Upper Peninsula where he experienced poor connectivity. To remedy this, his data during the trip, from 9/20/2015 - 9/22/2015, has been removed from analysis.
\end{enumerate}
See ``Data Management Decisions.docx'' on Box for more details.
...
\noindent After eliminating the above individuals, \textbf{\Sexpr{sum(!users$exclude)}} remain in the sample. The included participants were all on-study for at least \textbf{\Sexpr{max(days) + 1}} days.
...
\item Participant 18 traveled to the Upper Peninsula for \Sexpr{sum(daily$travel[daily$user == 18])} days and experienced poor connectivity while there. These days were 
...
We notice large spikes of missingness for participant 14 and 35 in~\Cref{fig:missing-step-plot}.
\begin{itemize}
  \item It was previously believed that participant 14 was on vacation in Hawaii and did not bring her Jawbone. This could not be confirmed by GPS data (see~Cref{sec:changes}).
  \item Participant 35 has no Jawbone data after 18 days on-study. He dropped out of communication with study organizers; we believe this is due to a lost Jawbone. We do have Google Fit data which, when Jawbone data is available, is highly correlated with Jawbone step count 
...
print(xtable(estimate(model4), floating = FALSE, digits = 3,
             ...
             label = "tab:actsed-marginal-model"), 
      sanitize.rownames.function = identity)
@
...
\begin{equation*}
Y_{t+1} \sim \alpha_{0} + \alpha_{1} Z_{t} + \beta_{1} \left(A_{1, t} - 0.3\right) + \beta_{2} \left(A_{2, t} - 0.3\right), \quad \text{if } I_{t} = 1,
  \tag{\ref{eqn:actsed-marginal-model} revisited}
\end{equation*}
...
print(xtable(estimate(model2.sens1, normal = FALSE),
             ...
             label = "tab:model-with-day-effect-sens1", floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model4.sens1$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens1, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after removing participant 35",
             label = "tab:actsed-marginal-model-sens1"),
      sanitize.rownames.function = identity, table.placement = "H")
...
names(model3.sens1$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      ...
                                      "$\\beta_{2}$", "$\\beta_{3}$",
                                      "$\\beta_{4}$")
print(xtable(estimate(model3.sens1, normal = FALSE), floating = TRUE, digits = 3,
...
Next, we remove the original travel days recorded for participant 14 (9/18/2015 - 9/24/2015, see~\Cref{sec:changes}). This removal is \textit{in addition to} removing data from 9/1/2015 - 9/5/2015, when GPS data indicates the participant was in Hawaii. Note that after excluding these days, participant 14 was on-study for \Sexpr{length(unique(sens6$study.day.nogap[sens6$user == 14]))} days, which is \Sexpr{ifelse(length(unique(sens6$study.day.nogap[sens6$user == 14])) < max(days) + 1, paste("below the threshold for"), paste("long enough for"))} inclusion in the primary analysis.

<<sens6-rmID14, results = "asis">>=
names(model1.sens6$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$", "$\\beta_{1}$")
print(xtable(estimate(model1.sens6, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-no-day-effect} after removing original travel dates for participant 14",
             label = "tab:model-no-day-effect-sens6",
             floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model2.sens6$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\alpha_{2}$", "$\\beta_{1}$",
                                      "$\\beta_{2}$")
print(xtable(estimate(model2.sens6, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-with-day-effect} after removing original travel dates for participant 14",
             label = "tab:model-with-day-effect-sens6", floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model4.sens6$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens6, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after removing original travel dates for participant 14",
             label = "tab:actsed-marginal-model-sens6"),
      sanitize.rownames.function = identity, table.placement = "H")

names(model3.sens6$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\alpha_{2}$", "$\\beta_{1}$",
                                      "$\\beta_{2}$", "$\\beta_{3}$",
                                      "$\\beta_{4}$")
print(xtable(estimate(model3.sens6, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-model} after removing original travel dates for participant 14",
             label = "tab:actsed-model-sens6"), 
      sanitize.rownames.function = identity, table.placement = "H")
@

\subsection{Increase Minimum-Required Time-on-Study}
\label{sec:sens-analysis-min-time-on-study}
In the primary analysis, participants were included if they remained on-study for at least \Sexpr{max(days) + 1} days. Here, we increase this required time-on-study to exclude more participants. We present model results as above when we require 37, 38, and 40 days on-study. \textit{Note that participant 35 is included in these analyses.}
...
print(xtable(estimate(model1.sens2, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-no-day-effect} after requiring 37 days on-study",
...
print(xtable(estimate(model2.sens2, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-with-day-effect} after requiring 37 days on-study",
             label = "tab:model-with-day-effect-sens2", floating = FALSE, digits = 3), 
      sanitize.rownames.function = identity)

names(model4.sens2$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens2, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after requiring 37 days on-study",
             label = "tab:actsed-marginal-model-sens2"),
      sanitize.rownames.function = identity, table.placement = "H")
...
print(xtable(estimate(model3.sens2, normal = FALSE), floating = FALSE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-model} after requiring 37 days on-study",
...
print(xtable(estimate(model1.sens3, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-no-day-effect} after requiring 38 days on-study",
...
print(xtable(estimate(model2.sens3, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-with-day-effect} after requiring 38 days on-study",
             label = "tab:model-with-day-effect-sens3", floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model4.sens3$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens3, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after requiring 38 days on-study",
             label = "tab:actsed-marginal-model-sens3"),
      sanitize.rownames.function = identity, table.placement = "H")
...
print(xtable(estimate(model3.sens3, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-model} after requiring 38 days on-study",
...
print(xtable(estimate(model1.sens4, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-no-day-effect} after requiring 41 days on-study",
...
print(xtable(estimate(model2.sens4, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-with-day-effect} after requiring 41 days on-study",
             label = "tab:model-with-day-effect-sens4", floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model4.sens4$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens4, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after requiring 41 days on-study",
             label = "tab:actsed-marginal-model-sens4"),
      sanitize.rownames.function = identity, table.placement = "H")
...
print(xtable(estimate(model3.sens4, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-model} after requiring 41 days on-study",
...
print(xtable(estimate(model1.sens5, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-no-day-effect} after requiring 42 days on-study",
...
print(xtable(estimate(model2.sens5, normal = FALSE),
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:model-with-day-effect} after requiring 42 days on-study",
             label = "tab:model-with-day-effect-sens5", floating = TRUE, digits = 3), 
      sanitize.rownames.function = identity, table.placement = "H")

names(model4.sens5$coefficients) <- c("$\\alpha_{0}$", "$\\alpha_{1}$",
                                      "$\\beta_{1}$",  "$\\beta_{2}$")
print(xtable(estimate(model4.sens5, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-marginal-model} after requiring 42 days on-study",
             label = "tab:actsed-marginal-model-sens5"),
      sanitize.rownames.function = identity, table.placement = "H")
...
print(xtable(estimate(model3.sens5, normal = FALSE), floating = TRUE, digits = 3,
             caption = "Fitted coefficients and univariate Hotelling's T tests for Model~\\labelcref{eqn:actsed-model} after requiring 42 days on-study",
