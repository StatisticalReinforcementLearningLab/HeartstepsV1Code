...
<<missing-step-plot, fig.height=3, out.width="\\textwidth">>=
text(barplot(height = user.nojb, names.arg = user.ids, ylim = c(0, 27),
...
<<stepsmodel-0, results = "asis", cache = T>>=
...
\subsection*{Model 2}
\begin{equation}
  Y_{t+1} \sim \alpha_{0} + \alpha_{1} d(t) + \alpha_{2} Z_{t} + \beta_{0} \left(A_{t} - 0.6\right) + \beta_{1} d(t) \left(A_{t} - 0.6\right), \quad \text{if } I_{t} = 1,
...
<<stepsmodel-1, results = "asis", cache = T>>=
...
<<timemodel, results = "asis", cache = T>>=
timefit1 <- fit(jbsteps30.log ~ jbsteps30pre.log)
names(timefit1$coef) <- c("$\\alpha_{0}$", "$\\alpha_{1}$")
print(xtable(estimate(timefit1, ztest = FALSE), floating = FALSE, digits = 3,
             caption = "Fitted Coefficients for Model~\\ref{eqn:resid-model}"), 
      sanitize.rownames.function = identity)
@

<<timemodel-resid, fig.cap = "Plot of Residuals vs. Study Day with Loess Smoothers with Varying, Small Bandwidths ", fig.pos="H">>=
bandwidth <- c(.071, .3)
sample <- sample(1:length(timefit1$residuals), ceiling(.35 * length(timefit1$residuals)))

par(mfrow = c(1, length(bandwidth)), mar = c(0, 0, 2, 0), oma = c(4, 4, 1, 1), 
    mgp = c(2, 0.6, 0), font.main = 1)

for (i in 1:length(bandwidth)) {
  plot(timefit1$residuals[sample] ~ eval(d)$study.day.nogap[sample], axes = F,
       xlab = "", ylab = "", main = "", type = "p")
  box(col = "grey40")
  axis(side = 1, outer = T, cex = .6)
  lines(with(eval(d), 
             predict(loess(timefit1$residuals ~ study.day.nogap,
                span = bandwidth[i]))),
        col = color, lwd = 6)
  if (i == 1) {
  ...
  title(main = paste("Bandwidth:", bandwidth[i]), cex = .3)
}
@
...
Here, we again run model~\ref{eqn:resid-model}, first using only availble times for which $A_{t} = 1$ and then again using available times for which $A_{t} = 0$. We use LOESS smoothing on the residuals from each model separately, then take the difference to inform the functional form of the time-treatment interaction.

<<timemodel-interaction, results = "asis", cache = T>>=
timefit2 <- fit(jbsteps30.log ~ jbsteps30pre.log, data = subset(eval(d), send == T))
names(timefit2$coef) <- c("$\\alpha_{0}$", "$\\alpha_{1}$")
print(xtable(estimate(timefit2, ztest = FALSE), floating = FALSE, digits = 3,
             caption = "Fitted Coefficients for Model~\\ref{eqn:resid-model} when $A_{t} = 1$"), 
      sanitize.rownames.function = identity)

timefit3 <- fit(jbsteps30.log ~ jbsteps30pre.log, data = subset(eval(d), send == F))
names(timefit3$coef) <- c("$\\alpha_{0}$", "$\\alpha_{1}$")
print(xtable(estimate(timefit3, ztest = FALSE), floating = FALSE, digits = 3,
             caption = "Fitted Coefficients for Model~\\ref{eqn:resid-model} when $A_{t} = 0$"), 
      sanitize.rownames.function = identity)
@

<<timemodel-interaction-resid-1, fig.pos = "H", include = F>>=
sample <- sample(1:length(timefit2$residuals), ceiling(.35 * length(timefit2$residuals)))

par(mfrow = c(1, length(bandwidth)), mar = c(0, 0, 2, 0), oma = c(4, 4, 1, 1), 
    mgp = c(2, 0.6, 0), font.main = 1)

for (i in 1:length(bandwidth)) {
  plot(timefit2$residuals[sample] ~ eval(d)$study.day.nogap[eval(d)$send == T][sample], axes = F,
       xlab = "", ylab = "", main = "", type = "n")
  box(col = "grey40")
  axis(side = 1, outer = T, cex = .6)
  lines(with(eval(d), 
             predict(loess(timefit2$residuals ~ study.day.nogap[send == T],
                span = bandwidth[i])) - 
               predict(loess(timefit3$residuals ~ study.day.nogap[send == F],
                span = bandwidth[i]))),
        col = color, lwd = 6)
  if (i == 1) {
    axis(side = 2, outer = T, cex = .6)
...
<<spaghettiplot, fig.cap="Mean difference in proximal step count following a suggestion vs. folllwing no suggestion, by particpant. The y-axis is truncated below at -3000 steps", fig.align = "H">>=
...
<<availplot, out.width=".7\\textwidth">>=
...
<<activesedmod, include = F>>=
suggfit <- fit(jbsteps30.log ~ jbsteps30pre.log )
names(timefit1$coef) <- c("$\\alpha_{0}$", "$\\alpha_{1}$")
print(xtable(estimate(timefit1, ztest = FALSE), floating = FALSE, digits = 3), 
      sanitize.rownames.function = identity)
@
