...
We call a decision ``valid'' if it occurred within an intended timeslot (e.g., morning commute, etc.) and was not duplicated. In total, there were \Sexpr{dim(suggest)[1]} ``valid'' decisions made throughout the study. Of those, we eliminate \Sexpr{table(suggest$avail, suggest$send)[1,2]} which occrred while the participant should have been marked as unavailable according to the above definition. 

The following plot depicts the number of users on-study at each decision point, after removing travel dates:
<<decisionplot>>=
...
We first regress step count in the 30 minutes following a decision point against a centered treatment indicator, controlling for step count in the 30 minutes prior to the decision point. 

<<step-descriptives, include = FALSE>>=
user.days <- aggregate(start.udate ~ user, data = jawbone, FUN = unique)
ids <- unique(suggest$user)
sapply(ids, function(x) {
  sum(!(unique(suggest$study.date[suggest$user == x & suggest$travel == F]) %in% 
          user.days$start.udate[[ifelse(x < 10, paste('0', x, sep = ""), paste(x))]]))
})
@
...
# days <- 1:42
# fit <- function(y) {
#   d <- subset(suggest, study.day.nogap %in% days)
#   y <- substitute(y)
#   y <- eval(y, d)
#   d <- d[!is.na(y), ]
#   d$send.cent <- as.factor(as.numeric(d$send) - .6)
#   # ids <- with(subset(suggest, !is.na(y)))
#   fit <- geeglm(y ~ jbsteps30pre + send.cent, id = user,
#                 weights = as.numeric(avail), data = d, scale.fix = TRUE)
#   combos <- do.call("rbind",
#                     lapply(days,
#                            function(x) grepl(paste("^send.cent0.4(|.+\\)", x, ")$",
#                                                    sep = ""), names(fit1$coef))))
#   list(est = estimate(fit, combos), n = length(fit$geese$clusz))
# }
# f30 <- fit(jbsteps30)
# f60 <- fit(steps60.spl)
@ 
