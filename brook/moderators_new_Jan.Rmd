...
```{r,fig.width=9,fig.height=3.5,out.width='0.9\\textwidth',fig.align='center',fig.cap='Exponentially weighted past 7-day proportion of thumbs up or down messages for a few users.'}
 suggest.analysis %>%
...
```{r,fig.width=9,fig.height=3.5,out.width='0.9\\textwidth',fig.align='center',fig.cap='Exponentially weighted past 7-day proportion of thumbs up or down ACTIVE messages for a few users.'}
suggest.analysis %>%
  filter(user %in% c(1,10,14)) %>%
...
```{r, fig.width=9,fig.height=3.5,out.width='0.9\\textwidth',fig.align='center',fig.cap='Square root of average daily step count from previous 7 days for a few users. With exponentially weighted versions.'}
suggest.analysis %>%
  filter(user %in% c(1,10,14)) %>%
  select(user,decision.index.nogap,steps.window7.sqrt.avg,
         steps.window7.sqrt.avg.exp05, steps.window7.sqrt.avg.exp1) %>%
  melt(id=c('user','decision.index.nogap'))%>%
  mutate(user = factor(user,levels=c(1,10,14),
                       labels=c('User 1','10','14')))%>%
  ggplot(aes(x=decision.index.nogap,y=value)) +
...
The exponentially weighted 7-day standard deviation is displayed below for a few users.
```{r,fig.width=8.5,fig.height=5.5,out.width='\\textwidth',fig.cap='Exponential weighting of previous 7-day standard deviation of the log steps in the 60-minute window surrounding each decision point.',warning=FALSE}
suggest.analysis %>%
  filter(user %in% c(1,10,14,25,30)) %>%
  select(user,decision.index.nogap,window7.steps60.log.sd,
         window7.steps60.log.sd.exp05, window7.steps60.log.sd.exp1) %>%
  melt(id=c('user','decision.index.nogap'))%>%
  mutate(user = factor(user,levels=c(1,10,14,25,30),
                       labels=c('User 1','10','14','25','30')))%>%
  ggplot(aes(x=decision.index.nogap,y=value)) +
  geom_line(aes(color=variable)) +
  facet_wrap(~user) +
  scale_color_brewer(palette='Set1',name='') +
