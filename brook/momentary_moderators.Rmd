...
## To fix:
# need to handle slots where no steps were accumulated
# need to calculate length of slot for those cases
# e.g. no steps before first decision point of day after 5 a.m.
#   still want the time between 5 a.m. and that decision point
# or just subtract 5 a.m. from each decision point!
jbslotpre.cutoff <-
  jbslotpre %>%
  filter(as.POSIXct(start.datetime) >= as.POSIXct(start.date) + hour_cutoff*60*60 &
           study.date == start.date)
...
jbslotpre.cumulative <-
  ...
  summarise(study.date=unique(study.date),
            ...
            slot_time_min = difftime(max(decision.utime),
                                     min(start.utime), units='mins')) %>%
  right_join(
  ...
  #mutate(cum_day_steps = cumsum(slot_steps),
  #       cum_day_min = cumsum(as.numeric(slot_time_min))) 
