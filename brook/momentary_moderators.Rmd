...
## Indicator of zero cumulative step rate up to decision ponit
jbslotpre.final$step_rate_0_indicator <- 
  as.numeric(with(jbslotpre.final, step_rate_cumulative_hour==0))
jbslotpre.final$step_rate_nonzero_indicator <-
  with(jbslotpre.final, 1 - step_rate_0_indicator)
...
```{r cumulative-model, echo=FALSE, include=FALSE}
...
```{r cumulative-model-results, echo=FALSE, results='asis', include=FALSE}
...
## Indicator of zero prior step rate
Let $Q_t = 0$ if $\tilde{R}_t=0$ and $Q_t=1$ otherwise.

$$
Y_{t+1} \sim \alpha_0 + \alpha_1\tilde{Z}_t + \alpha_2\tilde{R}_t + \beta_1Q_t(A_t - 0.6) + \beta_2 (1 - Q_t) (A_t - 0.6)
$$

```{r mod-cumsteps4, echo=FALSE}
mod.cumsteps4 <-
  geeglm(
    jbsteps30.log ~ jbsteps30pre.log.cen + step_rate_cumulative_log_cen +
      step_rate_0_indicator:I(send - 0.6) + 
      step_rate_nonzero_indicator:I(send - 0.6),
    id = user,
    corstr='independence',
    weights = as.numeric(jbslotpre.final$avail),
    scale.fix = TRUE,
...
vcov.cumsteps4 <- vcov.heartsteps.bgl(mod.cumsteps4, small=T)
se.cumsteps4 <- diag(vcov.cumsteps4)
coef.cumsteps4 <- coef(mod.cumsteps4)

test.cumsteps4 <- pointwise.table.small(coef.cumsteps4,
                               vcov.cumsteps4,
                               n=length(mod.cumsteps4$geese$clusz),
                               alpha=0.05)
```

```{r mod-cumsteps4-results, echo=FALSE, results='asis'}
tt <- cbind('Estimate' = coef.cumsteps4,
            "SE" = sqrt(se.cumsteps4),
            test.cumsteps4)
rownames(tt) <- names(coef.cumsteps4)

coefNames4.cumsteps <-
  c("(Intercept)", "jbsteps30pre.log.cen", "step_rate_cumulative_log_cen",
    "step_rate_0_indicator:I(send - 0.6)",
    "I(send - 0.6):step_rate_nonzero_indicator")
tt <- tt[coefNames4.cumsteps, ]
...
$$
Y_{t+1} \sim \alpha_0 + \alpha_1\tilde{Z}_t + \alpha_2 d(t) + \alpha_3 \tilde{R}_t + \beta_1 Q_t(A_t - 0.6) + \beta_2 (1 - Q_t) (A_t - 0.6) + \beta_3 d(t) (A_t - 0.6)
$$

```{r mod-cumsteps5, echo=FALSE}

mod.cumsteps5 <-
  geeglm(
    jbsteps30.log ~ jbsteps30pre.log.cen + study.day.nogap + 
      step_rate_cumulative_log_cen +
      step_rate_0_indicator:I(send - 0.6) + 
      step_rate_nonzero_indicator:I(send - 0.6) + 
      study.day.nogap:I(send - 0.6),
    id = user,
...
vcov.cumsteps5 <- vcov.heartsteps.bgl(mod.cumsteps5, small=T)
se.cumsteps5 <- diag(vcov.cumsteps5)
coef.cumsteps5 <- coef(mod.cumsteps5)

test.cumsteps5 <- pointwise.table.small(coef.cumsteps5,
                               vcov.cumsteps5,
                               n=length(mod.cumsteps5$geese$clusz),
                               alpha=0.05)

```

```{r mod-cumsteps5-results, echo=FALSE, results='asis'}
tt <- cbind('Estimate' = coef.cumsteps5,
            "SE" = sqrt(se.cumsteps5),
            test.cumsteps5)
rownames(tt) <- names(coef.cumsteps5)

coefNames5.cumsteps <-
  c("(Intercept)", "jbsteps30pre.log.cen", "study.day.nogap",
    "step_rate_cumulative_log_cen",
    "step_rate_0_indicator:I(send - 0.6)",
    "I(send - 0.6):step_rate_nonzero_indicator", "study.day.nogap:I(send - 0.6)")
tt <- tt[coefNames5.cumsteps, ]
