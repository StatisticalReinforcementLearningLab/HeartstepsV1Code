...
$$Y_{t+1} \sim \alpha_0 + \alpha_1 Z_t + \alpha_2 d(t) + \beta_1(A_t - 0.6) + \beta_2 d(t) (A_t - 0.6) + \beta_3 Z_t(A_t-0.6) + \beta_4 d(t) Z_t$$
```{r zt-mod1-time, echo=FALSE}
m1.zt.time <- 
  geeglm(
    jbsteps30.log ~ jbsteps30pre.log + study.day.nogap + I(send - 0.6) +
      study.day.nogap:I(send-0.6) +
      jbsteps30pre.log:I(send-0.6) + study.day.nogap:jbsteps30pre.log,
  id = user, 
  corstr = 'independence',
  weights = as.numeric(suggest.analysis$avail),
  scale.fix = TRUE,
...
jbslotpre.cutoff <-
  jbslotpre %>%
  filter(as.POSIXct(start.datetime) >= trunc(as.POSIXct(start.date), 'day') +
           hour_cutoff*60*60 & study.date == start.date) %>%
  inner_join(select(suggest.analysis, user, study.date, decision.index.nogap),
             by=c('user'='user',
...
jbslotpre.cumulative <-
  ...
  right_join(
    select(suggest.analysis, user, decision.index.nogap, study.date,
           study.day.nogap, jbsteps30pre.log, jbsteps30.log,
           decision.utime) %>%
      mutate(study.date.starttime = 
           trunc(as.POSIXct(study.date), 'day') + hour_cutoff*60*60),
    by=c('user'='user','decision.index.nogap'='decision.index.nogap',
         'study.date'='study.date')
  ) %>% ungroup %>%
  mutate(decision.daytime.hours = 
           difftime(decision.utime, study.date.starttime, units='hours'))

## WRONG
## FIX:
##  use gmtoff to get the local decision time
##  then subtract 5 a.m. on that day from the local decision time
## also need to investigate user 14 and see whether the decision timestamps are accurate
...
jbslotpre.cumulative <-
  ...
  mutate(steps_cumulative_log = log(steps_cumulative + 0.5),
         step_rate_cumulative_hour = steps_cumulative / as.numeric(decision.daytime.hours))
