...
Let $U_t$ be an indicator of whether $Z_t = 0$. 
...
$$
\begin{aligned}
Y_{t+1} \sim \alpha_0 &+ \alpha_1 \tilde{Z}_t + \alpha_2 d(t) \\
...
In this model, $\beta_3$ is the difference in the active message treatment effect when $U_t=1$ and $U_t=0$. Similarly, $\beta_4$ is the difference in the sedentary message treatment effect when $U_t=1$ and $U_t=0$.
...
```{r plot-decision-timestamps, echo=FALSE, include=FALSE}
...
$$ Y_{t+1} \sim \alpha_0 + \alpha_1\tilde{R}_t + \alpha_2 \tilde{Z}_t + \beta_1(A_t - 0.6) + \beta_2\tilde{R}_t(A_t-0.6)$$
