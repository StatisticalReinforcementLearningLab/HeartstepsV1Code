...
## Indicator of zero prior cumulative steps
Let $Q_t = 0$ if and only if $\tilde{R}_t=0$ (when the cumulative step rate up to time $t$ is zero).

$$
Y_{t+1} \sim \alpha_0 + \alpha_1\tilde{Z}_t + \beta_1Q_t(A_t - 0.6) + \beta_2 (1 - Q_t) (A_t - 0.6)
$$

```{r mod-cumsteps4, echo=FALSE}
mod.cumsteps4 <-
  geeglm(
    jbsteps30.log ~ jbsteps30pre.log.cen + step_rate_0_indicator:I(send - 0.6) + 
...
$$
Y_{t+1} \sim \alpha_0 + \alpha_1\tilde{Z}_t + \alpha_2 d(t) + \beta_1 Q_t(A_t - 0.6) + \beta_2 (1 - Q_t) (A_t - 0.6) + \beta_3 d(t) (A_t - 0.6)
$$
