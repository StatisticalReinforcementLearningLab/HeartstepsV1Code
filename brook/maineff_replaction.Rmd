...
gf_impute_summary %>% 
  ...
  xlab('Percent of decision points\nwith missing Jawbone step count') +
...
## Sensivity analyses
The following sensitivity analyses are also in the primary analysis document. For brevity, all of the results below use the Google Fit imputed data. The Jawbone-only analyses are in the primary analysis document.
...
### Removing user 35
User 35 was removed for the sensitivity analyses in the primary analysis document. After imputing Google Fit step counts, most of user 35's decision points are not missing. The above, imputed analyses can be compared to the analyses in section 8.1 of the primary analysis document, in which user 35 is removed.

### Remove user 14 travel days
User 14 was initially marked as travelling from 9/18/15 to 9/24/15, although GPS seems to indicate that user 14 was in Ann Arbor. The primary analyses include the days 9/18 to 9/24. These analyses exclude 9/18 to 9/24.
...
There are `r length(users.on37)` users in this analysis.
