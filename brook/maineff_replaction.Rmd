...
```{r model2, echo=FALSE, cache=TRUE}
...
```{r time1, echo=FALSE, fig.width=6.5, fig.height=5}
...
#pointwise.table.small(coef(mainmod1), S=vcov.heartsteps.bgl(mainmod1,small = T),
#                      n=length(mainmod1$geese$clusz))


plotdf <- data.frame(
  resid = mainmod1$y - mainmod1$fitted.values,
  send = mainmod1$data$send,
  study.day.nogap = mainmod1$data$study.day.nogap,
  avail = mainmod1$data$avail
)
...
plot.resid.time1 <- 
  ...
  geom_point(aes(color=send), shape=1) + 
