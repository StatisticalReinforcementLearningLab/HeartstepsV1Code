...
se1.brook <- diag(vcov.heartsteps.bgl(mod1.brook, small=T))
...
se2.brook <- diag(vcov.heartsteps.bgl(mod2.brook, small = TRUE))
tt <- cbind('Estimate' = coef(mod2.brook),
            "SE" = sqrt(se2.brook))
            #"SE (Audrey's code)" = sqrt(aud1))
...
This model implies the following relationship between the raw step count before and after a decision point over the course of the study.
```{r plot-mod2-print, echo=FALSE, fig.width=7, fig.height=6}
