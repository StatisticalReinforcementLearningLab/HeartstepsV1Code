...
% to address any knitr issues, start here:
...
imgdev <- ifelse(Sys.info()["sysname"] == "Windows", "pdf", "tikz")
opts_chunk$set(fig.align="center", dev=imgdev, dev.args=list(pointsize=11), echo=FALSE, fig.width=6.5, fig.show="hold", par=TRUE)
...
<<users, results="asis">>=
  d <- with(users,
            cbind("Male" = gender == "male",
                  ...
                  "Using Personal Phone" = own.phone == TRUE))
...
<<baselineactivity, results="asis">>=
    d <- with(users,
              cbind("Num. days walking >10 min in last week" = walk10.days.intake,
                    "Time walking on one day (min)" = walk.hrs.intake * 60 +
                      walk.min.intake,
                    "Time sitting on one weekday (hr)s" = sit.hrs.intake + 
                      sit.min.intake / 60,
                    "Num. days with vigorous activity in last week" = 
                      vigact.days.intake,
                    "Duration of vigorous activity in one day (min)" = 
                      vigact.hrs.intake * 60 + vigact.min.intake,
                    "Num. days with moderate activity in last week" = 
                      modact.days.intake,
                    "Duration of moderate activity in one day (min)" = 
                      modact.hrs.intake * 60 + modact.min.intake
              ))
  d <- cbind(apply(d, 2, mean, na.rm = TRUE),
             apply(d, 2, sd, na.rm = TRUE))
  colnames(d) <- c("Mean", "Std. Dev.")
  print(xtable(d, digits = c(0, 1, 1), caption = "Baseline activity levels"),
...
The following plots depict the daily step count for each user, with the solid series corresponds to Jawbone and dotted to Google Fit. Breaks coincide with one or more days on which no step counts were recorded that day. The \Sexpr{color.name} segments indicate that planning was carried out the day before. The horizontal reference line gives the user average daily Jawbone step count and extends up to the known or projected exit date. Vertical lines correspond to days on which no part of the EMA was completed (apart from possibly planning), with the dotted line type used for an inactive connection at the EMA notification slot.
  ...
  with(d, sapply(index,
                 function(j)
                   with(subset(d, index == j)[c(seq(1, sum(index == j), by = by),
                                                sum(index == j)), ],
                        points(end.utime, sum.steps, type = "s",
