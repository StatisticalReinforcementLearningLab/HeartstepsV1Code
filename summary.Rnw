...
% chunks are typically followed by paragraph breaks, which are non-trivial to
% omit from the "knitted" LaTeX document
...
d <- cbind(apply(d, 2, sum, na.rm = TRUE), apply(d, 2, sum, na.rm = TRUE)
