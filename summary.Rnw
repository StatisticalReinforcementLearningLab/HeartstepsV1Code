...
The Response table captures suggestions to which the user did not provide a response, so we can infer the randomized status for users connected and presumably available at the decision time slot. For the $\Sexpr{with(suggest, sum(is.na(notify)))}$ decision points with no connection, we could consider the corresponding users to be unavailable for the intervention.

<<suggest-avail, results="asis">>=
suggest$avail <- "Available"
suggest$avail[!(suggest$recognized.activity %in% c("STILL", "UNKNOWN"))] <-
  "Snooze disabled, but in transit"
suggest$avail[suggest$snooze.status] <- "Snooze enabled"
suggest$avail[!suggest$connect] <- "Disconnected"
d <- with(suggest, table(avail, !is.na(response), useNA = "ifany"))
colnames(d) <- c("No", "Yes")
d <- cbind(c("Message in Response?", rownames(d)), rbind(colnames(d), d))
rownames(d) <- NULL
print(xtable(d, digits = 0, align = rep("r", ncol(d) + 1)),
      floating = FALSE, booktabs = TRUE, hline.after = c(-1, 1, nrow(d)),
      include.rownames = FALSE, include.colnames = FALSE,
      sanitize.text.function = identity)
@

The Response suggestion status generally agrees with availability at the decision time slot, as-defined on the basis of snooze being disabled and the recognized activity classification being STILL or UNKNOWN. However note that, among those available, the proportion presumably randomized to have a suggestion sent is $\Sexpr{with(suggest, round(sum(avail == "Available" & !is.na(response))/sum(avail == "Available"), 2))}$, which is higher than the anticipated $40\%$.
