...
jawbone$user <- gsub("(heartsteps\\.test|@gmail.*$)", "", jawbone$userid, perl = TRUE)
@
...
<<steps, echo = FALSE, out.width = "\\textwidth", fig.align = "center">>=
sapply(sort(unique(jawbone$user)), function(u)
  sapply(sort(unique(with(jawbone, end.datetime.yday[user == u]))), function(d)
    with(subset(jawbone, user == u & end.datetime.yday == d),
         plot(end.utime, cumsum(steps), type = "s",
                             main = paste("User", x)))))
@
