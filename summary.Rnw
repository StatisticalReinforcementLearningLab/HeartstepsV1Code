...
\item $\Sexpr{with(daily, sum(!connect))}$ cases where the user's phone has no active connection. We could consider the user unavailable for the planning intervention.
...
with(subset(suggest, slot != time.stamp.slot),
     plot(index, user.index, pch = 16, cex = 0.75, frame.plot = FALSE,
