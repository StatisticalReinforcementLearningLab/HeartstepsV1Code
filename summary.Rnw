...
\midrule
Structured{\_}Planning, Unstructured{\_}Planning & User reviewed/revised response & Discard earlier or next-day duplicate plans \\
\midrule
EMA{\_}Response & User reviewed/revised response & Discard earlier duplicate responses \\
\midrule
EMA{\_}Completed & Unknown & Take the maximum completion status among duplicates \\
\midrule
Momentary{\_}Decision & Unknown if time stamps among duplicates are the same. Otherwise see \cref{sec:slot} & Discard duplicate decisions that are completely redundant (by time, message content, etc.), don't correctly link to Response, or take place in the wrong time slot \\
\bottomrule
\end{tabular}
...
The EMA{\_}Context{\_}Notified table provides the randomized planning status, but this often does not correspond to the planning status administered to the user when we link to the Structured{\_}Planning and Unstructured{\_}Planning tables by any reasonable identifier. This is due to a race condition in which the EMA{\_}Context{\_}Notified planning status stored on the phone is read before being updated. So its value might represent either an initial or previous status value.

We conjecture that this issue does not affect the actual, \emph{administered} planning status; the information available in Structured{\_}Planning and Unstructured{\_}Planning appears to reflect the randomized value. However we do not know the randomized planning status among those who did not 
